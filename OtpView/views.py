        ...
        try:
            ...
            if serializer.is_valid():
                ...
                SendMessages.objects.create(
                    otp_id=otp, otp=random_otp, created_at=int(time.time()))
...
def home(request):
    ...
    listotp = []
    for row in Otp.objects.all():
        listotp.append({
            "id": str(row.id),
            "username": row.username
        })

    context = {"data": json.dumps(listotp)}
...
def send(request):
    if request.method == "POST":
        try:
            url = os.getenv('SWAGGER_SERVER') + 'otpsend'
            header = {
                "Content-Type": 'application/json'
            }
            data = {
                'email': 'bhargav@bhargav1.com',
                'mobile_number': '7296909049',
                'isd_code': '+91'
            }
